class String
  ...
  def to_area
    if Esdm::Area.state_or_territory?(self)
      @area_codes = Esdm::Area.area_codes.find_all {|row| row[1].upcase == self }.map {|a| a.first }
    end
  end
  ...
  def to_region(options = {})
    if self.to_s.length == 3  # an area code
      row = Esdm::Area.area_codes.find {|row| row.first == self.to_s }
      return row.last if row
    elsif self.to_s.length == 5
      if row = Esdm::Area.zip_codes.find {|row| row.first == self.to_s }
  ...
  def to_zip
    if self.match(',')
      ...
      @zip_codes = Esdm::Area.zip_codes.find_all {|row| row[1] && row[1].downcase == area[0].downcase and row[2].downcase == area[1].downcase }.map {|a| a.first }
    else
      @zip_codes = Esdm::Area.zip_codes.find_all {|row| row[1] != nil and row[1].downcase == self.downcase }.map {|a| a.first }
    end
  end
  ...
  def to_gmt_offset
    if Esdm::Area.zip_or_territory?(self.to_s)
      row = Esdm::Area.zip_codes.find {|row| row[2] != nil and (row[2].upcase == self.to_s.upcase or row[0] == self.to_s) }
      row[5] if row
    end
  end
  ...
  def to_latlon
    if Esdm::Area.zip?(self)
      row = Esdm::Area.zip_codes.find {|row| row.first == self.to_s }
      row[3] + ', ' + row[4] if row
    end
  end
